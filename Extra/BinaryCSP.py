...
class QueenConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		if value1[0] == value2[0] or value1[1] == value2[1]: 
			return False
		if abs(int(value1[0])-int(value1[1])) == abs(int(value2[0])-int(value2[1])): 
			return False
		return True
...
def consistent(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.var1 == var:
			other = assignment.assignedValues[constraint.var2]
			if constraint.isSatisfied(value, other) == False:
				return False
		if constraint.var2 == var:
			other = assignment.assignedValues[constraint.var1]
			if constraint.isSatisfied(other, value) == False:
				return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	for value in orderValuesMethod(assignment, csp, var):
		old = assignment.assignedValues[var]
		if consistent(assignment, csp, var, value):
			...
			if result != None:
				return result
		assignment.assignedValues[var] = old
	return None
		...
		if not assignment.isAssigned(var):
			...
			for constraint in csp.binaryConstraints:
				if constraint.affects(var) and not assignment.isAssigned(constraint.otherVariable(var)):
					degree += 1
			remains = len(domains[var])
			if remains < minRemain:
				minRemain = remains
				nextVar = var
			if remains == minRemain and degree > maxDegree:
				maxDegree = degree
				nextVar = var
	return nextVar
"""
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	temp = {}
	for value in values:
		temp[value] = getConstraintValue(assignment, csp, var, value)
...
def getConstraintValue(assignment, csp, var, value):
	counter = 0
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			for otherValue in assignment.varDomains[constraint.otherVariable(var)]:
				if otherValue != None:
...
def forwardChecking(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			for otherValue in domains[constraint.otherVariable(var)]:
				if not constraint.isSatisfied(value, otherValue):
					inferences.add((constraint.otherVariable(var), otherValue))
	for inference in inferences:
		domains[inference[0]].remove(inference[1])
		if len(domains[inference[0]]) == 0:
			for inference in inferences:
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	while len(q) != 0:
		currConstraint = q[0]
		q.remove(currConstraint)
		var1 = currConstraint[0]
		var2 = currConstraint[1]
		infer = revise(assignment, csp, var1, var2, currConstraint[2])
		if infer == None:
			for inference in list(inferences):
				domains[inference[0]].add(inference[1])
			return None
		if len(infer) != 0:
			for constr in csp.binaryConstraints:
				if constr.affects(var2):
					q.append((var2, constr.otherVariable(var2), constr))
			inferences = set(inferences.union(infer))
	return inferences
...
def AC3(assignment, csp):
	...
	q = []
	for constraint in csp.binaryConstraints:
		q.append((constraint.var1, constraint.var2, constraint))
		q.append((constraint.var2, constraint.var1, constraint))
	while len(q) != 0:
		currConstraint = q[0]
		q.remove(currConstraint)
		var1 = currConstraint[0]
